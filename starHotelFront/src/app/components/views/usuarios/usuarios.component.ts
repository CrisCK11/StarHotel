...
export class UsuariosComponent implements OnInit {
  ...
  guardar(formulario: any): void {
    if (this.formUsuarios.valid) {
      this._usuariosService.registrarUsuario(formulario).subscribe(
        (response) => {
          this.cerrarModal();
          this.obtenerUsuarios();
          this.resetForm();
  ...
  obtenerUsuarioPorId(id: any) {
    ...
    this._usuariosService.listarUsuarioPorId(id).subscribe(
      (response) => {
        if (response) {
          form.controls['apellido'].setValue(
            response.usuarios.apellido || null
          );
          form.controls['contrasena'].setValue(
            response.usuarios.contrasena || null
          );
          form.controls['direccion'].setValue(
            response.usuarios.direccion || null
          );
          ...
          form.controls['rol']
            .get('idRol')
            ?.setValue(response.usuarios.rol?.idRol || null);
          form.controls['telefono'].setValue(
            response.usuarios.telefono || null
          );
        } else {
      ...
      if (result.isConfirmed) {
        this._usuariosService.eliminarUsuarios(id).subscribe(
          (response) => {
            Swal.fire(
              'Eliminado!',
              'El usuario ha sido eliminada con éxito.',
              'success'
            );
            this.obtenerUsuarios();
          },
  ...
  editar(id: number, formulario: any): void {
    if (this.formUsuarios.valid) {
      this._usuariosService.editarUsuarios(id, formulario).subscribe(
        (response) => {
          this.cerrarModal();
          this.obtenerUsuarios();
          this.resetForm();
